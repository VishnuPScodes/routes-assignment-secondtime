...
function SeafoodDetails({image,name}) {
  ...
  return (
    <div>
        <div id='s-d'>
          {data.length!=0 && <img src={data?.meals[0]?.strMealThumb} height={'100%'} width='100%' alt='' /> }
        </div>
        <h3> {data.length!=0 && <h3>{data.meals[0].strMeal}</h3>}</h3>
        <p>Instructions : {data.length!=0 && <div style={{width:"60%",margin:'auto'}}>{data.meals[0].strInstructions}</div>}</p>
    </div>
  )
}
